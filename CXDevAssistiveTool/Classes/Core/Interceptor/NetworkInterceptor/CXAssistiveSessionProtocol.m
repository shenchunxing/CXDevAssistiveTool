...
static NSString *kUrlProtocolKey = @"CXAssistiveSessionProtocol";

@interface CXAssistiveSessionProtocol ()<NSURLSessionDataDelegate>
...
@implementation CXAssistiveSessionProtocol

+ (CXAssistiveURLSessionDemux *)sharedDemux {
    
    static dispatch_once_t      sOnceToken;
    static CXAssistiveURLSessionDemux * sDemux;
    dispatch_once(&sOnceToken, ^{
        NSURLSessionConfiguration *     config;
        
        config = [NSURLSessionConfiguration defaultSessionConfiguration];
        sDemux = [[CXAssistiveURLSessionDemux alloc] initWithConfiguration:config];
    });
    return sDemux;
}
...
+ (BOOL)canInitWithRequest:(NSURLRequest *)request {
    ...
    if (![CXNetworkInterceptor shareInterceptor].shouldInterceptor) {
        return NO;
    }
...
- (void)stopLoading {
    ...
    [[CXNetworkInterceptor shareInterceptor] interceptorDidReceiveData:self.responseData task:self.task startTime:self.startTime];
    if (self.task != nil) {
        [self.task cancel];
    }
}
