...
@interface CXAssistiveCrashPlugin ()
{
    //文件路径
...
@implementation CXAssistiveCrashPlugin

+ (instancetype)sharedPlugin {
    
    static CXAssistiveCrashPlugin *_instance;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _instance = [[CXAssistiveCrashPlugin alloc] init];
    });
    return _instance;
}
...
- (void)install {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [CXAssistiveSignalCrashHandler registerSignalHandler];
        [CXAssistiveExceptionCrashHandler registerExceptionHandler];
    });
}
