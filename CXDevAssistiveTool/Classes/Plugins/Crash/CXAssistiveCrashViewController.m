//
//  CXAssistiveCrashViewController.m
//  CXDevAssistive
//
...
@interface CXAssistiveCrashViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation CXAssistiveCrashViewController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CXAssistiveCrashCell *cell = [tableView dequeueReusableCellWithIdentifier:@"CXAssistiveCrashCell"];
    if (!cell) {
        cell = [[CXAssistiveCrashCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"CXAssistiveCrashCell"];
    }
...
- (NSArray<UITableViewRowAction *> *)tableView:(UITableView *)tableView editActionsForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (![infoDict[@"resolve"] boolValue]) {
        UITableViewRowAction *action_fix = [UITableViewRowAction rowActionWithStyle:UITableViewRowActionStyleNormal title:@"修复" handler:^(UITableViewRowAction * _Nonnull action, NSIndexPath * _Nonnull indexPath) {
            ...
            [[CXAssistiveCrashPlugin sharedPlugin] replaceCrashLogToFileByKey:key withDict:infoDict];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CXAssistiveContentViewController *contentVC = [[CXAssistiveContentViewController alloc] init];
...
- (void)deleteRowAtIndexPath:(NSIndexPath *)indexPath forKey:(NSString *)key {
    ...
    UIAlertAction *confirmAction = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [[CXAssistiveCrashPlugin sharedPlugin] deleteCrashLogFromDateKey:key];
...
- (void)evaluateSutiationLevelAtIndexPath:(NSIndexPath *)indexPath forKey:(NSString *)key {
    ...
    UIAlertAction *simpleAction = [UIAlertAction actionWithTitle:@"一级" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [self updateSutiationLevelAtIndexPath:indexPath type:CXAssistiveCrashSituationLevelSimple forKey:key];
    }];
    UIAlertAction *mediumAction = [UIAlertAction actionWithTitle:@"二级" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [self updateSutiationLevelAtIndexPath:indexPath type:CXAssistiveCrashSituationLevelMedium forKey:key];
    }];
    UIAlertAction *difficuteAction = [UIAlertAction actionWithTitle:@"三级" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [self updateSutiationLevelAtIndexPath:indexPath type:CXAssistiveCrashSituationLevelDifficult forKey:key];
    }];
    [alertVC addAction:cancelAction];
...
- (void)updateSutiationLevelAtIndexPath:(NSIndexPath *)indexPath type:(CXAssistiveCrashSituationLevel)level forKey:(NSString *)key {
    
    NSMutableDictionary *infoDict = self.datas[indexPath.row];
    if (level == CXAssistiveCrashSituationLevelSimple) {
        [infoDict setObject:@0 forKey:@"level"];
    }else if (level == CXAssistiveCrashSituationLevelMedium) {
...
- (void)describeSutiationAtIndexPath:(NSIndexPath *)indexPath forKey:(NSString *)key {
    ...
    UIAlertAction *saveAction = [UIAlertAction actionWithTitle:@"保存" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        [[CXAssistiveCrashPlugin sharedPlugin] replaceCrashLogToFileByKey:key withDict:dict];
        [self.tableView reloadRowsAtIndexPaths:@[indexPath] withRowAnimation:UITableViewRowAnimationNone];
        self.tableView.editing = NO;
    }];
...
- (NSMutableArray *)datas {
    ...
    if (_datas == nil) {
        _datas = [NSMutableArray arrayWithArray:[[CXAssistiveCrashPlugin sharedPlugin] crashLogs]];
    }
    return _datas;
}
